...
variables:
  ...
  - name: judge_instructions
    ...
    kind: text
    required: false
    default: "Produce a single best answer that incorporates the strongest points from each candidate.  Where candidates disagree, name the disagreement and pick a side with a one-line rationale.  Cite candidates by index (#1, #2, ...)."
---
