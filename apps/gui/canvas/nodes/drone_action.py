"""FPV drone node - a deployed drone shown on the canvas.

Distinct from ``AgentNode``:

* AgentNode wraps a ``PersonalityCard`` template - a prototype used by
  the flow executor to dispatch a fresh single-shot run.
* DroneActionNode wraps a ``DroneAction`` - a deployed instance of a
  blueprint with its own transcript. Double-click to open a chat dialog
  scoped to that one action.

Inputs / outputs:

* No flow-execution ports - drones are anchored entities that exist on
  the canvas to make the conversation lineage visible.
...
class DroneActionNode(BaseNode):
    """A deployed FPV drone dropped onto the canvas."""
    ...
    def __init__(self, node_id: str, action: dict[str, Any]) -> None:
        ...
        super().__init__(
            node_id=node_id,
            title=str(snapshot.get("name") or "FPV Drone"),
    ...
    def refresh_visuals(self) -> None:
        ...
        self._title = str(action.get("name") or snapshot.get("name") or "FPV Drone")
