# AgentOrchestra â€” Retrospective Development Roadmap
...
Reaper Drone, lets the operator **drag those Reaper Drones onto a canvas** to see who can
...
(CLI, tests, MCP) are real but secondary â€” they exist to keep the desktop
product honest, not as the primary surface.

---

## 2. Phases shipped â€” actually-merged work

Twelve PRs merged to `main`. The work falls naturally into five phases.

### Phase 1 â€” MVP scaffold (PR #1)
...
autonomous dispatch, the EventBus + SSE, the Live + Review panes, the diff
artifact, and a Code-Edit card.

Honest assessment: PR #1 was *not* clean â€” it is a multi-sprint super-PR that
covers what should have been four or five commits, but it gives every later
PR a coherent foundation to extend.

### Phase 2 â€” multi-vendor + GUI shell (PRs #2 â€“ #5)

**Theme:** the subscription-only thesis becomes real.

- **PR #2** â€” `claude-cli` and `gemini-cli` provider adapters. The pivotal
...
- **PR #3** â€” wires `pyside6_annotator` overlay onto the main window;
  isolates the `pyside6_annotator` dep into its own optional extra so the
  base install stays small.
- **PR #4** â€” Composer hardening: the form is built from the template's
...
- **PR #5** â€” GUI polish: per-row actions on the History page (Replay /
...
Ollama autonomous adapters, per-card `fallbacks`, runs.replay, Claude hook
...
### Phase 3 â€” autonomous parity + sandbox (PRs #6 â€“ #7)

**Theme:** the operator is human, give them buttons.

- **PR #6** â€” first wave of operator scripts: `launch.cmd`, `stop.cmd`,
  ...
  named Reaper Drones, multi-turn chat, minimap, auto-layout, and undo.
- **PR #7** â€” ships `setup.cmd / launch.cmd / stop.cmd / update.cmd /
...
### Phase 4 â€” chat + canvas + agents + limits (PRs #8 â€“ #11)

**Theme:** the canvas becomes a workbench, not a demo.

- **PR #8** â€” `RpcClient` surfaces the service's actual error body on 5xx
  (no more silent `Internal Error` toasts).
- **PR #9** â€” slug-safe ephemeral card archetype on the Chat / Agents path
...
- **PR #10** â€” `stop.cmd` now reaps the supervisor-spawned service too
  (netstat-based port-listening kill on 8765, on top of the window-title
  kill).
- **PR #11** â€” the Phase-4 marquee: `canvas.create new conversations` +
...
### Phase 5 â€” attachments + repo-aware coding + audit hardening + UX mirror (PR #12)
...
1. **Image + Excel attachments end-to-end** â€” paperclip + drag-drop, 25 MB
   cap, Pillow-based image resizing, `.xlsx` / `.xls` / `.csv` rendered to
   markdown tables once at upload time, per-Reaper Drone storage + cross-agent
   auth checks on every attachment RPC.
2. **Repo-aware coding sessions** â€” bind an Agent to a Workspace, the CLI
   ...
   button; `Clone from gitâ€¦` dialog.
3. **Async-correctness audit** â€” lost-update + connection-sharing fixes
   across the store layer.
4. **Three audit batches** â€” A (HIGH security + crash + lifecycle, including
   a critical sandbox-injection fix and five HIGH leaks), B (MEDIUM UX +
   perf + budget), C (LOW drag-drop + shortcut + flow-cancel polish).
5. **Comprehensive README rewrite** â€” the 520-line operator-facing doc.
...
- **Single SQLite store with FTS5 + WAL** â€” every Run, Branch, Step,
  Approval, Outcome, Event, Reaper Drone, Attachment, Flow, FlowRun, Workspace,
...
- **Per-process boundary: GUI â†” service over local JSON-RPC + SSE.** The
  ...
  product trivially scriptable â€” every UI action is also an RPC.
- **Branch-per-Reaper Drone via `WorktreeManager`.** Every autonomous Run gets its own
...
- **Frozen presets registry (`apps/gui/presets.py`).** Single source of
  ...
  tab, the Canvas "+ New conversation" dialog, and the Reaper Drones tab "+ New
  Reaper Drone" dialog. Both registries are exported as tuples so a buggy consumer
  can't mutate them.

### Phase 6 â€” Canvas stability & type safety (PR #14)

**Theme:** technical debt remediation and performance.

- **Canvas Memory Lifecycle** â€” resolved dangling C++ signal connections, fixed cross-boundary race conditions during scene teardown, and eliminated memory bloat from abandoned draft edges.
- **LOD & Performance** â€” edge rendering now falls back to straight lines (`lineTo`) and skips labels/arrowheads at low zoom levels (<0.2) to save GPU cycles. Implemented a zero-shot repath timer to prevent CPU storms during multi-node drag events.
- **Dict-based Data Routing** â€” the `FlowExecutor` now resolves inputs as a dict keyed by target ports instead of a flat list, fulfilling the original design goal (Gap 1) and enabling complex multi-input agents (e.g. separating `instructions` from `context`).
- **Strict Typing (`mypy`)** â€” added a `type-check` CI job and resolved over 50 foundational typing errors across the project, including enforcing explicit generic arguments (`list[dict[str, Any]]`) and fixing protocol signature mismatches.
...
- **`apps/gui/presets`** â€” shared registry promotion. The Chat tab loses its
  ...
  The Reaper Drones tab "+ New Reaper Drone" dialog slices `MODEL_PRESETS` for
...
- **Mid-thread thinking / skills changes** show an amber hint that the
  ...
  creation).
- **Repo-aware coding sessions polish** â€” clone-from-git, live git status,
  project conventions inlining (#12 carryover that gets its final UX in
  #13).
- **Comprehensive QA hardening** â€” async-correctness, attachments lifecycle,
...
- **Worktree-bound dispatch via Claude Code CLI.** The `apps/service/dispatch/a2a.py`
  ...
  autonomous Runs can dispatch through the same Max-plan auth the chat path
  uses, instead of the API-keyed `anthropic` provider â€” closes the last
...
- **Per-agent context-window estimate ribbon.** The Limits tab already knows
  ...
  count. Surface the fraction-used directly on each Reaper Drone's chat dialog
  header so an operator sees "â‰ˆ 38 % of 200 K used" before the next send.
...
- **MCP server marketplace.** The MCP registry (Phase 3 / PR #7) already
  ...
  test-call button.
- **Multi-repo Reaper Drones.** Today a Reaper Drone binds to one workspace. Lift the
  constraint so a Reaper Drone can hold a list of `workspace_id`s, with the
...
- **PR comment ingestion.** Use the `gh` CLI to ingest PR review comments
  as `HandoffCard` events â€” closing the loop for the existing Tracker
  archetype.
...
- **Subscription rate limits, not API limits.** The Limits tab tells the
  ...
  remaining-budget number â€” an operator can hit a cap mid-flow with no
...
- **Canvas scaling beyond ~50 agents.** `QGraphicsView` is fast, but the
  ...
  Sugiyama on `networkx`. Both are O(nÂ²)-ish. We have no real-world data
  past ~20 nodes.
...
- **License is "Proprietary. See LICENSE (TBD)".** As long as that line
  stays in the README, no open-source contributor flow is possible â€” every
  external PR is legally ambiguous. A clear license decision is the
  unblocker for community contribution.

## ShadowLoop Status (2026-05-12)

- Phase 1: Shadow-Plan guard shipped (`PLAN.md`-gated autonomous mutation with explicit `403 Shadow-Plan Violation`).
- Phase 2: Mapper archetypes shipped (`ui-architect`, `logic-liaison`) with read-only mapping contracts.
- Phase 3: Analytics dashboard shipped with `analytics.summary` and `analytics.leaderboard`.
- Phase 4: Parallel consensus orchestration shipped, including candidate comparison in Review and winner selection persistence.